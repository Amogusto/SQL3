...
CREATE TABLE music (
    ...
    name VARCHAR(100),
    artist VARCHAR(100), 
    user_id BIGINT UNSIGNED NOT NULL, -- пользователь, который загрузил песню
    `size` INT,
    created_at DATETIME DEFAULT NOW(),
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
...
CREATE TABLE music_playlists (
	id SERIAL PRIMARY KEY,
    music_id BIGINT UNSIGNED NOT NULL, -- треки в плейлисте
    user_id BIGINT UNSIGNED NOT NULL, -- создатейль плейлиста
    created_at DATETIME DEFAULT NOW(),
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
...
DROP TABLE IF EXISTS playlists_library;
CREATE TABLE playlists_library (
    playlist_id BIGINT UNSIGNED NOT NULL,
    user_id BIGINT UNSIGNED NOT NULL, -- люди добавившие плейлист
    created_at DATETIME DEFAULT NOW(),
